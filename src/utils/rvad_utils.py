...
def enframe(speech, fs, winlen, ovrlen):
    ...
    N, flth, foVr = len(speech), int(
        np.fix(fs*winlen)),  int(np.fix(fs*ovrlen))
    ...
    idx = np.tile(np.arange(0, flth), (frames, 1)) + \
        np.tile(np.arange(0, (frames)*foVr, foVr), (flth, 1)).T
...
def snre_highenergy(fdata, nfr10, flen, fsh10, ENERGYFLOOR, pv01, pvblk):
    ...
    # energy estimation
    e = np.zeros(nfr10,  dtype='float64')
    e = np.insert(e, 0, 'inf')

    for i in range(1, nfr10+1):
        for j in range(1, flen+1):
            e[i] = e[i]+np.square(fdata_[(i-1)*fsh10+j])

        if np.less_equal(e[i], ENERGYFLOOR):
            e[i] = ENERGYFLOOR
    ...
    for i in range(1, int(np.floor(nfr10/NESEG))+1):
        ...
        if np.not_equal(i, 1):
            emin[range((i-1)*NESEG+1, i*NESEG+1)] = 0.9 * \
                emin[(i-1)*NESEG]+0.1*emin[(i-1)*NESEG+1]
    ...
    if np.not_equal(i*NESEG, nfr10):
        ...
        emin[range(i*NESEG+1, nfr10+1)
             ] = eY[int(np.floor((nfr10-(i-1)*NESEG)*0.1))]
    ...
    for i in range(1, int(np.floor(nfr10/NESEG))+1):
        # np.amax(Dsmth[range((i-1)*NESEG+1, i*NESEG+1)])
        Dsmth_max[range((i-1)*NESEG+1, i*NESEG+1)
                  ] = np.amax(e[range((i-1)*NESEG+1, i*NESEG+1)])

    if np.not_equal(i*NESEG, nfr10):
        # np.amax(Dsmth[range((i-1)*NESEG+1, nfr10+1)])
        Dsmth_max[range(i*NESEG+1, nfr10+1)
                  ] = np.amax(e[range((i-1)*NESEG+1, nfr10+1)])
    ...
    sign_vad = 0
    noise_seg = np.zeros(int(np.floor(nfr10/1.6)))
    noise_seg = np.insert(noise_seg, 0, 'inf')
    ...
    for i in range(1, nfr10+1):
        if (snre_vad[i] == 1) and (sign_vad == 0):  # % start of a segment
            ...
            if np.equal(sum(pv01_[range(nstart, nstop+1)]), 0):
                noise_seg[range(int(np.round(nstart/1.6)),
                                int(np.floor(nstop/1.6))+1)] = 1
                n_noise_samp = n_noise_samp+1
                noise_samp[n_noise_samp, :] = np.array(
...
def snre_vad(fdata, nfr10, flen, fsh10, ENERGYFLOOR, pv01, pvblk, vadThres):
    ...
    segsnr = np.zeros(nfr10)
    segsnr = np.insert(segsnr, 0, 'inf')
    segsnrsmth = 1
    sign_segsnr = 0
    D = np.zeros(nfr10)
    D = np.insert(D, 0, 'inf')
    postsnr = np.zeros(nfr10, dtype='float64')
    postsnr = np.insert(postsnr, 0, 'inf')
    snre_vad = np.zeros(nfr10)
    snre_vad = np.insert(snre_vad, 0, 'inf')
    sign_pv = 0
        ...
        if (pvblk_[i] == 1) and (sign_pv == 0):
            ...
            for j in range(nstart, nstop-1+1):  # previously it was for j=nstart:nstop-1
                for h in range(1, flen+1):
                    e[j] = e[j] + np.square(datai[(j-nstart)*fsh10+h])
                if np.less_equal(e[j], ENERGYFLOOR):
                    e[j] = ENERGYFLOOR
            ...
            tm1 = np.hstack(
                (np.ones(Dexpl)*D[nstart], D[range(nstart, nstop+1)]))
            ...
            Dsmth_thres = sum(Dsmth[range(
                nstart, nstop+1)]*pv01_[range(nstart, nstop+1)])/sum(pv01_[range(nstart, nstop+1)])
...
def frame_array_to_segments(frame_array):
    speech_segments = []
    aux_partial_segment = (0, 0, "NON_SPEECH")
    in_speech_segment = False
    for i, frame in enumerate(frame_array[3:]):
        adjusted_i = i + 3
        # Detectamos inicio de segmento de voz
        if frame_array[adjusted_i-3][1] == False and frame_array[adjusted_i-2][1] == True and frame_array[adjusted_i-1][1] == True and frame_array[adjusted_i][1] == True and not in_speech_segment:
            # Doy por terminado el segmento de NO_SPEECH anterior
            aux_partial_segment = (
                aux_partial_segment[0], frame_array[adjusted_i-2][0], "NON_SPEECH")
            speech_segments.append(aux_partial_segment)
            # Inicio de nuevo segmento de voz
            in_speech_segment = True
            aux_partial_segment = (frame_array[adjusted_i-2][0], 0, "SPEECH")
        # Detectamos fin de segmento de voz
        elif frame_array[adjusted_i-3][1] == True and frame_array[adjusted_i-2][1] == False and frame_array[adjusted_i-1][1] == False and frame_array[adjusted_i][1] == False and in_speech_segment:
            # Doy por terminado el segmento de SPEECH anterior
            aux_partial_segment = (
                aux_partial_segment[0], frame_array[adjusted_i-2][0], "SPEECH")
            speech_segments.append(aux_partial_segment)
            in_speech_segment = False
            # Inicio de nuevo segmento de NO_SPEECH
            aux_partial_segment = (
                frame_array[adjusted_i-2][0], 0, "NON_SPEECH")
        # Si es el ultimo frame cerramos el segmento
        elif adjusted_i == len(frame_array)-1:
            if in_speech_segment:
                aux_partial_segment = (
                    aux_partial_segment[0], frame_array[adjusted_i][0], "SPEECH")
            else:
                aux_partial_segment = (
                    aux_partial_segment[0], frame_array[adjusted_i][0], "NON_SPEECH")
            speech_segments.append(aux_partial_segment)

    segments = []
    for i, segment in enumerate(speech_segments):
        if segment[2] == "SPEECH":
            segments.append((segment[0]/1000, segment[1]/1000))
    return segments